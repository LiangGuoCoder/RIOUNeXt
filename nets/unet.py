...
class Unet(nn.Module):
    def __init__(self, num_classes=3, pretrained=False, backbone='EncoderNet', fuse_type='iAFF', fuse_type_1='AFF'):
        super(Unet, self).__init__()

        if backbone == "EncoderNet":
            self.resnet = Net(fuse_type=fuse_type, fuse_type_1=fuse_type_1)
            in_filters = [192, 512, 1024, 3072]
        else:
            raise ValueError('Unsupported backbone - `{}`, Use EncoderNet.'.format(backbone))
